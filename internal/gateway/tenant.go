...
//
// 또 실패 사유(헤더 없음/형식 오류/모르는 key)를 구분해서 알려주지 않는 이유는, 그 차이가 공격자에게 "이 key는 존재한다" 같은 정보를 흘리기 때문이다.
//
...
func (s *Server) resolveTenant(ctx context.Context, r *http.Request) (string, bool) {
	...
	//
	// key는 등록되어 있는데 값이 빈 문자열인 Secret 항목은 잘못 설정된 상태이며, 이때 ok=true를 주면 tenant 이름이 ""인 채로 인증이 통과해 버린다.
	//
