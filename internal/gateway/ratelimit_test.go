...
var _ = Describe("policyForTenant", func() {
	...
	//
	// 정책이 없는 tenant에 대해 nil을 조용히 돌려주면, 호출한 쪽이 nil을 역참조해 패닉이 나거나 "제한 없음"으로 오해해 모든 요청을 통과시킬 수 있다.
	//
...
var _ = Describe("bucketRegistry", func() {
	...
	//
	// 여기서 nil 검사를 빠뜨리면 nil 포인터 역참조로 게이트웨이가 패닉을 일으키거나, rpm=0으로 해석되어 무제한이어야 할 tenant의 모든 요청이 차단되는 정반대 동작이 된다.
