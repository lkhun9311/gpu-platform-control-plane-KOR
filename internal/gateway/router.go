...
import (
	...
	//
	// 문자열을 그대로 들고 다니지 않고 *url.URL로 돌려주는 이유는, 프록시(httputil.ReverseProxy)가 이 타입을 그대로 받기 때문이다.
	"net/url"
...
//
// 이 에러는 "조회 실패"가 아니라 "그런 model이 없음"이라는 정상적 결과 신호이므로, 상위 핸들러가 이걸 받아 404로 변환한다.
...
// 왜 0을 따로 처리하는가.
//
// spec.port에는 +kubebuilder:default=8080 마커가 붙어 있어서, API server를 거쳐 저장된 객체는 값이 비어 있어도 8080으로 채워져 들어온다.
//
// 그래서 운영 경로에서는 0이 나올 수 없다.
//
...
//
// 설계 근거(설계서 Identity model 절): 같은 model을 서빙하는 InferenceDeployment가 둘 이상인 것을 막을 수단이 없다.
//
// 그런 경우에도 요청이 backend 사이를 오가면 안 되므로, "가장 오래된 것이 이기고, 시각이 같으면 이름 오름차순"이라는 결정론적 규칙으로 딱 하나를 고르고 경고를 남긴다.
func (s *Server) backendFor(ctx context.Context, policy *platformv1.GPUQuotaPolicy, model string) (*url.URL, error) {
	...
	// client.InNamespace(...)로 정책이 지정한 namespace로 범위를 좁히고, client.MatchingFields{ModelNameIndex: model}로 미리 걸어 둔 인덱스를 조회한다.
	//
