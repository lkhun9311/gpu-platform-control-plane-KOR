...
// 1MB는 정상적인 chat completions 요청(수십 KB 수준)보다 넉넉히 크면서도
// 한 요청이 프로세스를 위협할 수 없는 크기다.
...
//
// 왜 본문을 복원해야 하는가:
// r.Body는 한 번만 읽을 수 있는 스트림이다. 여기서 model을 보려고 끝까지 읽어 버리면
// 뒤이어 프록시가 업스트림으로 보낼 때는 이미 다 읽힌 빈 스트림만 남는다.
// 그러면 모델 서버는 빈 본문을 받아 400을 돌려주는데, 게이트웨이는 그것을 그대로 전달할 뿐이라
// 원인이 게이트웨이에 있다는 사실이 드러나지 않는다.
...
func readModel(r *http.Request) (io.ReadCloser, string, error) {
	...
	//
	// 왜 나머지 필드를 무시하는가: 게이트웨이는 라우팅에 필요한 model만 알면 되고,
	// messages나 temperature 같은 나머지는 해석하지 않고 그대로 흘려보내야 한다.
	// 전체를 구조체로 정의하면 OpenAI API에 필드가 추가될 때마다 게이트웨이를 고쳐야 하고,
	// 정의하지 않은 필드가 조용히 사라져 업스트림에 전달되지 않는 사고가 난다.
	var probe struct {
...
//
// 왜 필요한가:
// 응답을 다 보낸 뒤 metric에 "어떤 코드로 끝났는지"를 남겨야 하는데,
// http.ResponseWriter에는 "방금 무슨 코드를 썼는지" 되묻는 방법이 없다.
...
type statusRecorder struct {
	http.ResponseWriter
	// code: 기록된 상태 코드다.
	// 200으로 초기화하는 이유는 핸들러가 WriteHeader를 부르지 않고 바로 Write만 하면
	// Go가 암묵적으로 200을 보내기 때문이다. 그 경우에도 metric에 200이 남아야 맞다.
	code int
}

// WriteHeader: 상태 코드를 기록한 뒤 원래 ResponseWriter로 그대로 넘긴다.
//
// Go 문법 설명: rec.ResponseWriter.WriteHeader(c)처럼 임베딩된 필드를 명시해 호출하면
// 우리가 덮어쓴 메서드가 아니라 원본의 메서드가 불린다.
...
// 임베딩은 인터페이스에 선언된 메서드만 물려준다. http.ResponseWriter에는 Flush가 없으므로,
// 이 래퍼는 http.Flusher를 만족하지 않게 된다.
// 리버스 프록시는 스트리밍을 하기 전에 응답 writer가 Flusher인지 확인하는데,
// 아니라고 판단하면 FlushInterval 설정과 무관하게 응답을 버퍼링해 버린다.
...
//
// 왜 30초인가: 모델 서버는 첫 토큰을 내놓기까지(prefill) 수 초가 걸릴 수 있으므로 짧게 잡으면 정상 요청이 잘린다.
// 반대로 무한정 기다리면 매달린 요청이 쌓여 게이트웨이의 연결이 고갈된다. 그 사이의 타협점이다.
...
// 0을 "설정하지 않음"으로 해석하는 이유는, 0을 그대로 Transport에 넘기면 http.Transport가 그것을
// "상한 없음(무한 대기)"으로 읽어 우리가 막으려던 바로 그 상황이 되기 때문이다.
func newReverseProxy(target *url.URL, responseHeaderTimeout time.Duration, onErr func(code int)) *httputil.ReverseProxy {
	...
	// 프록시가 응답을 모아 두었다가 한꺼번에 내보내면 클라이언트는 생성이 끝난 뒤에야 전부 받게 되어
	// 스트리밍이 무의미해진다.
	//
	// 다만 이 한 줄이 실제로 쓰이는 범위는 좁다. 정확히 알아 두는 편이 낫다.
	...
	// 모델 서버가 연결만 받아 두고 영영 응답하지 않으면 그 요청은 무한정 매달려 있고,
	// 그런 요청이 쌓이면 게이트웨이의 연결과 메모리가 고갈된다.
	//
	...
	// 이 둘은 운영상 전혀 다른 신호다.
	// 502는 backend Pod가 죽었거나 Service가 잘못 연결된 것이고,
	// 504는 Pod는 살아 있는데 너무 느리거나 매달린 것이다.
	// 대응이 다르므로 코드로 구분해 준다.
	p.ErrorHandler = func(w http.ResponseWriter, _ *http.Request, err error) {
		// 기본은 502다. 연결 거부, DNS 실패 등이 여기 해당한다.
