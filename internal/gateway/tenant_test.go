...
var _ = Describe("resolveTenant", func() {
	...
	// 이 테스트가 막는 회귀(설계서 Identity model 절): Authorization 헤더가 아예 없는 익명 요청이 통과하면 인증이 무의미해진다.
	//
	// 헤더가 없을 때 Header.Get이 빈 문자열을 주는데, 이를 제대로 걸러내지 못하면 빈 key로 조회가 이어지거나 tenant가 ""인 채로 통과할 위험이 있다.
	It("returns ok=false when the Authorization header is missing", func() {
		...
		// tenant도 함께 확인하는 이유는, ok=false인데 tenant에 값이 남아 있으면 호출한 쪽이 실수로 그 값을 써버릴 여지가 생기기 때문이다.
		//
		// BeEmpty()는 빈 문자열/빈 슬라이스/빈 map을 검사하는 gomega 단언이다.
		Expect(tenant).To(BeEmpty())
	})

	// 이 테스트가 막는 회귀(설계서 Identity model 절): Secret에 등록되지 않은 임의의 key로 인증이 통과하면 누구나 게이트웨이를 쓸 수 있다.
	//
	// map 조회의 두 값짜리 형태(ok)를 쓰지 않고 값만 받으면, 없는 key에 대해 제로값(빈 슬라이스)이 나오는데 이때 ok 검사가 없으면 tenant=""로 통과해 버린다.
	...
	// 이 테스트가 막는 회귀(설계서 Identity model 절): 위의 두 테스트는 전부 "거절"만 확인하므로, resolveTenant가 무조건 false를 돌려주는 망가진 구현이어도 통과해 버린다.
	//
	...
	// 이 테스트가 막는 회귀(설계서 Identity model 절): HTTP 인증 scheme은 RFC 7235상 대소문자를 구분하지 않는다.
	//
