...
// 이 테스트들은 Ginkgo(BDD 스타일 Go 테스트 framework)를 사용하며, Ginkgo 소개는 http://onsi.github.io/ginkgo/ 참고.
//
...
var _ = BeforeSuite(func() {
	...
	//
	// 이 등록을 빠뜨리면 아래 client.New는 성공해도 실제 NodeHealth를 Create할 때 "no kind is registered for the type" 에러가 나며 실패한다.
...
//
// ENVTEST 기반 테스트는 특정 binary에 의존하고 보통 controller-runtime이 정한 경로에 위치하며, Makefile target 없이(예: IDE로) 테스트를 직접 돌릴 때는 'BinaryAssetsDirectory'를 명시적으로 지정해야 함.
//
// 이 함수는 'KUBEBUILDER_ASSETS' 환경 변수 설정과 비슷하게 필요한 binary를 찾아 과정을 간소화하며, binary가 제대로 준비되도록 사전에 'make setup-envtest' 실행 권장.
//
// Go 문법 설명.
//
