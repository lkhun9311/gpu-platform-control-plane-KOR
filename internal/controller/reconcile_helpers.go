...
// 이렇게 하면 나중에 단계를 추가할 때 CRD를 다시 바꾸지 않아도 되고,
// 지금 시점에는 "구현하지 않은 상태를 status에 적어 거짓말하는" 일을 피할 수 있다.
const (
...
// meta.SetStatusCondition은 같은 type이 있으면 교체하고 없으면 추가하는 upsert를 대신 해 주고,
// status 값이 이전과 같으면 lastTransitionTime을 그대로 두는 규칙까지 표준대로 지켜 준다.
...
func setReadyCondition(status *platformv1.NodeHealthStatus, ready bool, reason, msg string, generation int64) {
	...
	// ObservedGeneration이 현재 generation보다 작으면 status가 아직 최신 spec을 반영하지 못한 것이므로,
	// 운영자와 도구가 "낡은 status를 보고 판단하는" 실수를 피할 수 있다.
...
// 안 바뀌었는데 Update를 보내면 불필요한 API 서버 write가 발생하고, 그 write가 다시 watch 이벤트를 일으켜
// reconcile이 자기 자신을 계속 깨우는 무한 루프가 된다.
