...
//
// 서빙 pod가 GPU 하나당 요청하는 node 확장 resource로, pod 수준 resource이며 GPUQuotaPolicy의 ResourceQuota 키와는 별개다.
...
const (
	...
	//
	// 하나의 InferenceDeployment가 소유한 pod를 선택하는 label로, Deployment의 selector는 생성 후 불변이라 한 번 설정하고 이후 변경하지 않는다.
...
//
// InferenceDeployment 본체는 읽기만(get/list/watch) 하고 status만 갱신하며, Deployment와 Service는 생성/수정까지 하지만 delete 권한은 없다(정리는 owner 참조를 통한 GC가 담당).
...
//
// 이 controller가 관리하는 field만 Deployment에 설정하고, selector는 생성 후 불변이라 한 번만 설정하고 이후 건드리지 않는다.
...
func computeInfDPhase(infd *platformv1.InferenceDeployment, dep *appsv1.Deployment) (string, metav1.Condition) {
	...
	//
	// condition의 ObservedGeneration은 "이 판정이 어느 spec generation 기준인지"를 남겨, 사용자가 오래된 condition을 최신인 것으로 오해하지 않게 한다.
	...
	// 1. stale gate, Deployment controller가 현재 spec을 관측할 때까지 phase 판단을 보류하고, spec이 0 replica를 원하고 status도 이미 0개를 보이는 경우에만 gate를 생략하는데, 빠질 replica가 없고 spec 의도가 확정적이기 때문이다.
	//
	...
	//
	// spec이 0을 원하고(의도가 확정적) status의 실제 replica도 이미 0이면(더 빠질 게 없음), status가 stale이든 아니든 결론은 어차피 "0개로 도달함"이라 바뀔 여지가 없다.
	//
	...
	//
	// 순서가 반대면 영원히 실패한 rollout을 계속 "Pending(곧 될 거예요)"으로 보고하게 되어, 운영자가 문제를 알아채지 못한다.
	//
	// Go 문법 설명.
	//
	...
	// 여전히 Progressing, scale-down 도중 남은 잉여 replica가 아직 제거되지 않은 상황을 막기 위해, 세 replica count가 모두 원하는 값과 같아야 한다.
	//
	// 왜 이 검사가 따로 필요한가.
	//
	// 위 5번은 전부 "미만(<)" 비교라서 개수가 남아도는 경우를 못 잡는다.
	//
	// replica를 3에서 2로 줄이는 중이라면 세 count가 모두 3일 수 있는데, 3 >= 2 이므로 5번을 그냥 통과해 버린다.
	//
...
//
// 결정적 실패를 Available=False의 Degraded로 status에 반영하고, DeploymentConflict가 낡은 ready count를 남기지 않도록 ReadyReplicas를 0으로 초기화한다.
...
func (r *InferenceDeploymentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		//
		// 이게 없으면 Deployment의 ReadyReplicas가 올라가도 아무도 알려주지 않아, status.phase가 Progressing에 멈춘 채 다음 이벤트를 기다리게 된다.
		//
