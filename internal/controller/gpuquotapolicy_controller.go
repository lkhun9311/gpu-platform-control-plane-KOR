...
const (
	...
	//
	// 확장 resource(nvidia.com/gpu)는 quota에서 requests.<resource> 형태의 key로 추적하며, local에서는 시뮬레이션한 nvidia.com/gpu 용량을 이 key로 제한한다.
	//
	// Go 문법 설명.
	//
...
//   - 반환값이 (ctrl.Result, error) 두 개인 이유는, 에러를 nil이 아닌 값으로 돌려주면 controller-runtime이 지수 백오프(exponential backoff)로 알아서 재시도해 주기 때문이다.
...
func (r *GPUQuotaPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 이번에 바뀐 정책을 읽고, 이미 삭제됐으면 IgnoreNotFound가 조용히 끝내준다.
	//
	// Go 문법 설명.
	//
	...
	// 표식이 없으면 먼저 붙이고 이번 pass는 종료하며, 소유 resource를 만들기 전에 정리 약속을 먼저 걸어야 중간에 삭제돼도 정리가 보장된다.
	//
	...
	// 원하는 상한 계산은 정책의 GPUCount만큼을 requests.nvidia.com/gpu로 제한하며, NOTE spec.gpuClass는 아직 class별로 시행하지 않고, 이 milestone은 class와 무관하게 단일 key(requests.nvidia.com/gpu)만 제한하므로, 같은 namespace를 겨냥한 gpuClass가 다른 두 정책이 같은 key를 제한하면, Kubernetes가 quota를 AND로 합쳐 가장 엄격한 쪽이 이기고, class별 key 분리는 시뮬레이션 용량 modeling 방식에 달려 보류하며 gpuClass는 기록만 하고 quota 범위는 안 정한다.
	//
	// Go 문법 설명.
	//
	...
	case apierrors.IsNotFound(err):
		...
		if err := r.Create(ctx, &rq); err != nil {
			if apierrors.IsAlreadyExists(err) {
				// 경쟁에서 밀림(동시 reconcile이나 informer 지연), 이제는 객체가 있으므로 실패시키지 않고 1초 뒤 다시 reconcile해 정상 경로로 흡수한다.
				//
	...
	case err != nil:
		...
		return ctrl.Result{}, err
	default:
		// 이미 있으면 이 정책이 소유한 게 맞는지부터 확인하고, 우리가 안 만든 남의 ResourceQuota를 덮어쓰면 남의 quota를 훼손하므로 가로채지 않고 Degraded로 보고한다.
		//
	...
	// 동기화 성공을 status에 멱등하게 기록하되, 복사본에 원하는 값을 채운 뒤 실제로 달라졌을 때만 저장한다.
	//
	// Go 문법 설명.
	//
...
// quotaName: 정책 이름으로 ResourceQuota 이름을 항상 같은 규칙으로 만들며, 같은 정책은 늘 같은 이름으로 mapping돼 중복 생성이나 추적 혼선을 막는다.
//
// Go 문법 설명.
//
...
// markDegraded: 결정적 실패를 Synced=False condition과 Degraded phase로 status에 기록하고, 막힌 조건이 풀리면 스스로 복구되도록 RequeueAfter로 재확인을 예약하며, Owns watch는 우리가 소유하지 않은 ResourceQuota 변화에는 안 울리므로 시간 기반 재확인이 필요하다.
//
// Go 문법 설명.
//
...
// setQuotaPhase: phase를 바꾸되 값이 실제로 달라질 때만 전환 시각을 갱신하며, 같은 phase를 다시 써도 전환 시각이 튀지 않게 하려는 것이다.
//
// Go 문법 설명.
//
...
// SetupWithManager: 이 controller를 Manager에 등록해 무엇을 지켜보고 무엇을 소유하는지 알려주며, For는 GPUQuotaPolicy가 바뀌면 reconcile하고 Owns는 우리가 만든 ResourceQuota가 바뀌어도 reconcile한다.
//
// Go 문법 설명.
//
