...
//
// mock이 아니라 진짜 API 서버를 쓰는 이유는 finalizer, status subresource, CRD validation, owner reference 같은 이 controller가 의존하는 동작이 전부 API 서버 쪽 기능이라 fake client로는 검증이 안 되기 때문이다.
package controller
...
var _ = Describe("GPUQuotaPolicy Controller", func() {
	...
	Context("When reconciling a resource", func() {
		...
		//   - 실제 클러스터에서는 Update가 만든 watch event가 다음 호출을 자동으로 유발하지만, 테스트는 Reconcile을 직접 부르므로 그 연쇄를 손으로 흉내 내야 한다.
		...
		It("syncs a ResourceQuota with the GPU ceiling and reports Synced", func() {
			...
			//
			// 바로 rq.Spec.Hard[gpuResource].Value()라고 쓰지 못하는 이유는, map 조회 결과는 주소를 얻을 수 없는 값이라 포인터 리시버 메서드인 Value()를 곧바로 부를 수 없기 때문이다(그래서 변수로 한 번 받는다).
		...
		//
		// ResourceVersion으로 확인하는 이유는, 이 값이 API 서버가 객체를 쓸 때마다 올려 주는 카운터라서 "값이 그대로 = 쓰기가 아예 일어나지 않았다"를 정확히 증명해 주기 때문이다.
		//
		...
		//
		// 그런 코드는 다른 팀이나 다른 도구가 관리하던 ResourceQuota를 조용히 훼손하는데, quota는 워크로드 스케줄링을 직접 막는 객체라 남의 상한을 바꾸면 남의 서비스가 죽는다.
		//
		...
		//
		// 왜 immutable이어야 하는가: 대상 namespace를 바꾸면 controller는 새 namespace에 quota를 만드는데, 옛 namespace의 quota는 이름이 정책 이름 기반이라 그대로 남아 아무도 정리하지 않는 고아가 된다.
		//
		...
		It("rejects a change to the immutable targetNamespace", func() {
			...
			// ContainSubstring은 문구 전체가 아니라 일부만 맞으면 되는 matcher이며, API 서버가 앞뒤로 덧붙이는 부가 정보에 영향받지 않게 해 준다.
