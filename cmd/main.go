...
import (
	...
	//
	// 모든 Kubernetes client 인증 plugin (Azure, GCP, OIDC 등)을 import해 exec-entrypoint와 run에서 사용할 수 있도록 보장한다.
...
var (
	...
	//
	// WithName("setup")을 붙이면 이 로거로 찍는 모든 로그에 logger=setup 필드가 따라붙는다.
	//
	// 그래서 나중에 로그를 볼 때 기동 로그와 reconcile 로그를 구분하기 쉬워진다.
...
//
// 전체 흐름은 (1)플래그 파싱 → (2)로거 설정 → (3)TLS/metrics/webhook 서버 옵션 구성으로 시작한다.
//
// 이어서 (4)Manager 생성 → (5)reconciler 등록 → (6)probe 등록 → (7)Manager 시작(블로킹) 순으로 진행된다.
//
...
func main() {
	...
	//
	// enable-http2 flag가 false(기본값)면 취약점 때문에 http/2를 비활성화해야 한다.
	//
	// 좀 더 구체적으로 http/2를 끄면 HTTP/2 Stream Cancellation 및 Rapid Reset CVE에 취약해지는 상황을 막을 수 있다.
	//
	// 자세한 내용은 아래를 참고한다:
	...
	//
	// metric endpoint는 'config/default/kustomization.yaml'에서 활성화하고, Metrics option으로 서버를 구성한다.
	//
	// 자세한 내용은 아래를 참고한다:
	...
	if secureMetrics {
		// FilterProvider로 metric endpoint를 authn/authz로 보호한다.
		//
		// 인가된 사용자와 서비스 계정만 metric endpoint에 접근하도록 보장한다.
		//
		// RBAC는 'config/rbac/kustomization.yaml'에서 구성한다.
		//
		// 자세한 내용은 아래를 참고한다:
	...
	// 인증서를 지정하지 않으면 controller-runtime이 metric 서버용 자체 서명 인증서를 자동 생성한다.
	//
	// 개발과 테스트에는 편리하지만 운영 환경에는 권장하지 않는다.
	//
	...
	//
	// Manager는 client, cache, metrics 서버, webhook 서버, probe 서버, leader election을 한데 묶어 생명주기를 함께 관리해 주는 컨테이너다.
	//
	// Go 문법 설명.
	//
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		//
		// 설계 근거: 컨트롤러는 클러스터 상태를 쓰기 때문에 두 replica가 동시에 같은 리소스를 reconcile하면 서로의 변경을 덮어쓰며 무한 루프를 만들 수 있다.
		//
		...
		//
		// LeaderElectionReleaseOnCancel은 Manager 종료 시 리더가 자발적으로 물러날지 정의한다.
		//
		// 이를 위해서는 Manager가 멈출 때 binary가 즉시 종료돼야 하고, 그렇지 않으면 이 설정은 안전하지 않다.
		//
		// 이 option을 켜면 새 리더가 LeaseDuration만큼 먼저 기다릴 필요가 없어 자발적 리더 전환이 크게 빨라진다.
		//
		// 기본 scaffold에서는 manager가 멈춘 직후 프로그램이 종료되므로 이 option을 켜도 무방하다.
		//
		// 다만 manager 종료 후 정리 작업 같은 어떤 동작을 수행하거나 수행할 예정이라면 사용이 안전하지 않을 수 있다.
