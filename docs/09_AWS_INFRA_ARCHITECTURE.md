...
| `gpu.yml` — **designed only, no such file exists** | would be `workflow_dispatch` only | Gated apply of `gpu_desired=0\                                                                                                                                                          | 1` — the **only** actor that scales the GPU node group (no autoscaler by design: one auditable switch, ephemeral windows) |
